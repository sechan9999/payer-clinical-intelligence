...
Around that sit an agent registry that publishes each agent's version, scope, and *autonomy grade*; an inline guardrail plugin that blocks prompt injection before the model is called; and an audit trail plus OpenTelemetry spans that record refusals with the same weight as successes.
...
**Google Cloud**: Cloud Run with scale-to-zero (`min_instances = 0`), Cloud SQL Postgres for state, Pub/Sub with an OIDC-authenticated push subscription, Secret Manager for database credentials, Model Armor for guardrails with a heuristic fallback, Cloud Trace for spans, and a 5-tab Streamlit Web Dashboard ([https://payer-clinical-intelligence.streamlit.app/](https://payer-clinical-intelligence.streamlit.app/)).

We kept one rule throughout: **the whole system runs offline with no credentials.** SQLite stands in for Cloud SQL and a heuristic screen stands in for Model Armor, so all 12 unit tests and 15 evaluation benchmark tests — including every access-control and human-gate assertion — run on a laptop with no cloud project. A reviewer can verify the claims before deciding whether to trust the demo.
...
**pgvector for retrieval.** The security boundary is already the right shape for vector search; only the ranking function changes over the permitted SQL candidate subset.
